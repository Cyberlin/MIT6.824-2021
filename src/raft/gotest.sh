...
if ! go test -c -o tester $(printenv -0 RACE); then
...
success=0
for f in $(ls test-*.log); do
    tail -n50 "$f" | grep -qL '^PASS$';
    if [ $? -eq 0 ]; then
		success=$((success+1))
    fi
done
((failed = logs - success))
echo current success: $success
...
for i in $(seq "$((success+failed+1))" "$runs"); do
	...
	if [[ -z "$test" ]]; then
		./tester -test.v &> "test-${i}.log" &
		pid=$!
	else
		{ time -p ./tester -test.run "$test" -test.v; } &> "test-${i}.log" &
		pid=$!
	fi
	# echo $pid > "test-${i}-$pid.pid"
...
echo real $(find . -name '*.log' | xargs tail -n 10 | grep -h 'real' | awk '{ SUM += $2 } END { print SUM / NR }')
echo user $(find . -name '*.log' | xargs tail -n 10 | grep -h 'user' | awk '{ SUM += $2 } END { print SUM / NR }')
echo sys $(find . -name '*.log' | xargs tail -n 10 | grep -h 'sys' | awk '{ SUM += $2 } END { print SUM / NR }')
